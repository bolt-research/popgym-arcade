...
config["MODEL_PATH"] = f"nips_analysis_128/PQN_RNN_{config['MEMORY_TYPE']}_{config['ENV_NAME']}_model_Partial={config['PARTIAL']}_SEED=0.pkl"
...
new_init_state = eqx.tree_at(lambda x: x.env_state.action_x, init_state, replace=jnp.array([6]))
new_init_state = eqx.tree_at(lambda x: x.env_state.action_y, new_init_state, replace=jnp.array([6]))
board = new_init_state.env_state.board.at[jnp.where(new_init_state.env_state.board == 2)].set(0).at[:, 1, 1].set(2)
...
# Compute the saliency maps
grads, obs_seq, grad_accumulator = get_saliency_maps(rng, model, config, max_steps=10, initial_state_and_obs=(new_init_state, new_init_obs))
# Visualize the saliency maps
vis_fn(grads, obs_seq, config, use_latex=True)