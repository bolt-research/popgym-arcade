...
class Skittles(environment.Environment[EnvState, EnvParams]):
    ...
    def render(self, state: EnvState) -> chex.Array:
        ...
        # def draw_life(i, canvas):
        #     heart_tl = (5 + i * 20, 5)  # Offset each heart by 20 pixels horizontally
        #     heart_br = (20 + i * 20, 20)  # 15x15 pixel heart
        #     return draw_heart(heart_tl, heart_br, jnp.array([1.0, 0.0, 0.0]), canvas)

        # canvas = jax.lax.fori_loop(
        #     0, jnp.minimum(state.lives, 5),  # Draw up to state.lives hearts, max 5
        #     draw_life,
        #     canvas
        # )
