...
class MBQNetworkRNN(eqx.Module):
    """CNN + MLP"""
    ...
    def __init__(self, key: PRNGKeyArray, obs_size: int, rnn_type: str = "lru"):
        ...
        self.Q = nn.Linear(in_features=256, out_features=self.action_dim, key=keys[6])
        self.R = nn.Linear(in_features=256, out_features=1, key=keys[8])
        self.Tr = nn.Linear(in_features=256, out_features=256, key=keys[8])
    ...
    def __call__(self, hidden_state, x, done, last_action):
        ...
        q_vals = eqx.filter_vmap(eqx.filter_vmap(self.trunk))(x)
        next_reward = eqx.filter_vmap(eqx.filter_vmap(self.R))(x)

        return hidden_state, (q_vals, next_reward)
