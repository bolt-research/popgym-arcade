...
def make_train(config):
    ...
    def train(rng):
        ...
        dummy_obs = jnp.zeros((config["OBS_SIZE"], config["OBS_SIZE"], 3), dtype=jnp.uint8)
        dummy_action = jnp.array(0, dtype=jnp.int32)
        dummy_target = jnp.array(0.0, dtype=jnp.float32)
        dummy_transition = {
            "obs": dummy_obs,
            "action": dummy_action,
            "target": dummy_target
        ...
        def _update_step(runner_state, unused):
            ...
            # Add to buffer
            flat_obs = transitions.obs.reshape(-1, *transitions.obs.shape[2:])
            flat_action = transitions.action.reshape(-1, *transitions.action.shape[2:])
            flat_target = lambda_targets.reshape(-1)
            ...
            # NETWORKS UPDATE
            def _learn_step(carry, _):
                train_state, rng = carry
                rng, key = jax.random.split(rng)

                batch = buffer.sample(train_state.buffer_state, key)
                minibatch_obs = batch.experience.first["obs"]
                minibatch_action = batch.experience.first["action"]
                target = batch.experience.first["target"]

                def _loss_fn(network):
                    q_vals = network(minibatch_obs)
            ...
            rng, _rng = jax.random.split(rng)
            num_updates = config["NUM_EPOCHS"] * config["NUM_MINIBATCHES"]
            (train_state, rng), (loss, qvals) = filter_scan(
                _learn_step, (train_state, rng), None, num_updates
            )
...
def single_run(config):
    alg_name = config.get("ALG_NAME", "DQN")
...
def tune(default_config):
    ...
    alg_name = default_config.get("ALG_NAME", "dqn")
