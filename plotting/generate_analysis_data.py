...
def analyze_model_saliency(config, seeds=[0, 1, 2, 3, 4], max_steps=200, visualize=True):
    """
    ...
    if visualize:
        ...
        plt.title(f"Terminal Saliency Distribution by Seed\n{config['MEMORY_TYPE']} on {config['ENV_NAME']}")
        plt.legend()
        plt.tight_layout()
        ...
        plt.title(f"Average Terminal Saliency Distribution Across Seeds\n{config['MEMORY_TYPE']} on {config['ENV_NAME']}")
        plt.legend()
        plt.tight_layout()
...
# Example usage
    ...
    for config in configs:
        print(f"Analyzing {config['MEMORY_TYPE']} on {config['ENV_NAME']} (Partial={config['PARTIAL']}, Seed={config['MODEL_SEED']})")
