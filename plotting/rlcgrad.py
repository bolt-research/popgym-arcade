...
def f(name):
    ...
    def plot_comparative_curves(runs_df, name):
        ...
        runs_df['EnvBaseName'] = runs_df['EnvName'].apply(lambda x: x.replace("Easy", "").replace("Medium", "").replace("Hard", ""))
        ...
        for env_base in envs:
            for partial_status in ['False', 'True']:
                    data = runs_df[(runs_df['EnvBaseName'] == env_base) & (runs_df['Partial'] == partial_status)]
                    if not data.empty:
