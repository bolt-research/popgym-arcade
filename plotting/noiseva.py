...
def evaluate(model, config):
    ...
    # Rollout
    noiseless_frames, _rng, normal_qvals = run_evaluation(_rng) # Shape (STEPS, 128, 128, 3), normal_qvals shape (STEPS, 2, n_actions)
    ...
    for noise_idx in range(1, num_noise + 1): # This is how many trajectories we want to generate
        ...
        log_key = "{}_{}_{}_model_Partial={}_SEED={}_NoiseIdx={}".format(
            config["TRAIN_TYPE"], config["MEMORY_TYPE"], config["ENV_NAME"],
            config["PARTIAL"], config["SEED"], noise_idx
        )
    ...
    def plot(noisy_frames, noiseless_frames, normal_qvals, last_qs):
        ...
        for idx in range(num_noise):
            ...
            axes[idx + 1, 3].set_title(rf'$Q(s_{{{num_noise}}}, a_{{{num_noise}}})$', fontsize=20)
    ...
    def batch_plot(noisy_frames, noiseless_frames, normal_qvals, last_qs):
        ...
        for batch_idx in range(num_batches):
            ...
            for row_idx in range(batch_rows):
                ...
                if idx == 0:
                    # first row
                    axes[row_idx, 0].imshow(np.array(noiseless_frames[0], dtype=np.uint8))
                    axes[row_idx, 0].set_title(r'$O_0$', fontsize=20)
                    axes[row_idx, 1].imshow(np.array(noiseless_frames[1], dtype=np.uint8))
                    axes[row_idx, 1].set_title(r'$O_1$', fontsize=20)
                    axes[row_idx, 2].imshow(np.array(noiseless_frames[-1], dtype=np.uint8))
                    ...
                    axes[row_idx, 0].imshow(np.array(noiseless_frames[0], dtype=np.uint8))
                    axes[row_idx, 0].set_title(r'$O_0$', fontsize=20)
                    axes[row_idx, 1].imshow(np.array(noisy_frames[idx - 1], dtype=np.uint8))
