...
def get_policy_saliency_map(
    ...
    """
    Computes saliency maps for PPO policy at every timestep.

    At each step t the gradient of sum(pi.logits[-1]) w.r.t. all past
    observations [obs_0, ..., obs_{t-1}] is computed by replaying the
    full observation sequence from a freshly initialised carry, matching
    the per-timestep semantics of get_saliency_maps.

    Args:
        seed: JAX PRNG key for reproducible randomization
        model: Pre-trained PPO ActorCriticRNN model
        config: Configuration dictionary with ENV_NAME, PARTIAL, OBS_SIZE
        max_steps: Maximum number of steps to generate saliency for;
                   stops early if the episode terminates
        initial_state_and_obs: Optional (env_state, obs) tuple; if None,
                               the environment is reset from seed

    Returns:
        grads: List of gradient arrays. grads[t] has shape (t+1, H, W, C),
               showing how each past observation influenced the policy at
               step t.
        obs_seq: JAX array of shape (T+1, H, W, C) containing all
                 observations collected during the rollout.
        grad_accumulator: List of cumulative gradient sums. Element t has
                          shape (H, W, C) and equals sum(grads[t], axis=0).
    """
    seed, _rng = jax.random.split(seed)
    env, env_params = popgym_arcade.make(
        config["ENV_NAME"], partial_obs=config["PARTIAL"], obs_size=config["OBS_SIZE"]
    ...
    def step_env_and_compute_grads(env_state, obs_seq, done_seq, key):
        def policy_logits_fn(obs_batch, done_batch):
            # Reinitialise carry from scratch to replay the full sequence
            a_state, c_state = model.initialize_carry(key=key)
            a_state = add_batch_dim(a_state, 1)
            c_state = add_batch_dim(c_state, 1)
            # (T, H, W, C) -> (T, 1, H, W, C); (T,) -> (T, 1)
            obs_in = add_batch_dim(obs_batch, 1, axis=1)
            done_in = add_batch_dim(done_batch, 1, axis=1)
            _, _, pi, _ = model(a_state, c_state, (obs_in, done_in))
            # Derive action for env step from stop-gradiented policy
            pi_sg = lax.stop_gradient(pi)
            action = pi_sg.logits[-1].squeeze(axis=0).argmax(axis=-1)
            step_key, _ = jax.random.split(key)
            new_obs, new_state, _, new_done, _ = step_fn(step_key, env_state, action)
            # Gradient target: sum of policy logits at the latest timestep
            return pi.logits[-1].squeeze(axis=0).sum(), (new_state, new_obs, new_done)
        ...
        obs_seq = jnp.concatenate(
            [obs_seq, new_obs[jnp.newaxis, :].astype(jnp.float32)]
        )
        done_seq = jnp.concatenate([done_seq, new_done[jnp.newaxis]])
    ...
    for _ in range(max_steps):
        seed, rng = jax.random.split(seed)
        grads_obs, env_state, obs_seq, done_seq = jax.jit(
            step_env_and_compute_grads
