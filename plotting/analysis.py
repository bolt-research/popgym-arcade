...
def process_run(run):
    ...
    try:
        ...
        history = history.drop_duplicates(subset=['true_steps'])
...
for (alg, env, partial), group in runs_df.groupby(['Algorithm', 'EnvName', 'Partial']):
    ...
    if len(all_steps) != INTERP_POINTS:
        ...
        continue
    # 🛠️ Pivot to ensure step-run alignment
    ...
    # 🛠️ Ensure normalized steps match exactly
    scale_factor = 200 / group['EnvMaxStep'].iloc[0]
    stats_df['StepsNormalized'] = stats_df['Steps'] * scale_factor
    ...
    try:  
        ...
        print(f"Skipping {alg}-{env}-{partial}: {str(e)}")

# Add this after creating interpolated_data
for env_group in interpolated_data:
    assert env_group['Steps'].nunique() == INTERP_POINTS, f"Misalignment in {env_group['EnvName'].iloc[0]}"
    
final_df = pd.concat(interpolated_data, ignore_index=True)

def extract_base_name(env_name):
    """Extract the base name of the environment by removing difficulty suffixes"""
    for suffix in ["Easy", "Medium", "Hard"]:
        if env_name.endswith(suffix):
            return env_name[:-len(suffix)]
    return env_name

def extract_difficulty(env_name):
    """Extract difficulty level from environment name"""
    if "Easy" in env_name:
        return 0
...
def plot_comparative_curves(df):
    ...
    envs['BaseName'] = envs['EnvName'].apply(extract_base_name)  # Extract base name
    envs['Difficulty'] = envs['EnvName'].apply(extract_difficulty)  # Extract difficulty
...
# Generate final plot
# Check alignment for a representative group
sample_group = final_df[
    (final_df.Algorithm == "PQN (MLP)") & 
    (final_df.EnvName == "CountRecallEasy")
]
assert sample_group['Steps'].nunique() == INTERP_POINTS, "X-values misaligned!"
plot_comparative_curves(final_df)
print(final_df[['Algorithm', 'EnvName', 'Partial', 'StepsNormalized', 'Smoothed', 'Lower', 'Upper']].drop_duplicates().sample(5))